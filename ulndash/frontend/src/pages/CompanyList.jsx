...
useEffect(() => {
  ...
  async function handleImport(formData) {
    try {
      const res = await CompaniesAPI.importSpreadsheet(formData);
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          <button onClick={()=>{ setEditing(null); setOpenForm(true); }} className="px-3 py-2 bg-blue-950 text-white rounded-lg">Add Company</button>
          <ImportSpreadsheetButton label="Import CSV / XLSX" onUpload={handleImport} />
          <Link to="/import" className="px-3 py-2 rounded-lg bg-gray-800 text-gray-200 text-sm border border-gray-700 hover:bg-gray-700">
            Import page
          </Link>
        </div>
      </div>
      ...
      <div className="text-white rounded-xs shadow overflow-auto">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            ...
            {!loading && rows.length === 0 && <tr><td colSpan="8" className="p-4 text-center">No companies</td></tr>}
            {!loading && rows.map(r=>(
              <tr key={r.id} className="border-t">
                ...
                <td className="p-3">
                  <EngageActions
                    phone={r.contact_phone || (['phone','whatsapp'].includes(r.contact_method) ? r.contact_person : null)}
                    whatsapp={r.contact_whatsapp || (r.contact_method === 'whatsapp' ? (r.contact_phone || r.contact_person) : null)}
                    email={r.contact_email || (r.contact_method === 'email' ? r.contact_person : null)}
                  />
                </td>
                <td className="p-3 text-right space-x-2">
                  <button onClick={()=>{ setEditing(r); setOpenForm(true); }} className="px-3 py-1 bg-gray-500 rounded">Edit</button>
